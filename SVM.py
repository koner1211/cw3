...
feature_names = ('dissimilarity', 'constrast', 'correlation', 'energy', 'homogeneity', ' ASM')
...
def get_full_names():
    ...
    return ['_'.join(f) for i in product(feature_names, dist_str, angles_str)]
...
for category, infile in zip(categories, files):
    ...
    xy = np.random.randint(0, img.height-size[1], 10)
    base_name, _ = splitext(infile)
    for i, (x, y) in enumerate(zip(xr, xr)):
        img_sample = img.crop((x, y, x+size[0], y+size[1]))
        img_sample.save(
            join(sample_folder, category, f'{base_name:s}_{i:o2d}.jpg'))
        img_grey = img.covert('L')
